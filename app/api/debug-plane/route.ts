...
export async function GET() {
  ...
  const hasSlug = !!process.env.PLANE_WORKSPACE_SLUG;
  const slug = process.env.PLANE_WORKSPACE_SLUG ?? "(missing)";
  ...
  if (!project) {
    return NextResponse.json({ message: "No projects need syncing.", hasApiKey, hasSlug, slug });
  }
  ...
  try {
    ...
    return NextResponse.json({ success: true, project: project.title, planeProjectId, hasApiKey, hasSlug, slug });
  } catch (err) {
